...
class UserControllers {
    ...
    public function __construct($conn) {
        ...
        public function Login(){
            if ($_SERVER['REQUEST_METHOD'] == 'POST') {
                $mail = $this -> package -> filtrerMail($_POST['mail']);
                $password = $this -> package -> filtrer($_POST['password']);
                if ($this -> user -> login($this->conn,$mail,$password)) {
                    $id = $this -> user -> getIDbyMail($this->conn,$mail);
                    $role = $this -> user -> getRole($this -> conn, $id);
                    $this -> historique -> Insert($this->conn, $id);
                    if ($role == 'rh') {
                        Route::redirect('/rh/dashboard');
                    }elseif ($role == 'security') {
                        Route::redirect('/qhse/dashboard');
                    }elseif ($role == 'admin') {
